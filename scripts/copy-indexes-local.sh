#!/usr/bin/env bash
#
# Copies OpenSearch indexes from MXP staging to a local cluster (localhost:9200).
# Uses scroll + bulk API since the clusters are in separate VPCs.
#
# Usage:
#   ./scripts/copy-indexes-local.sh                          # copy all 3 indexes
#   ./scripts/copy-indexes-local.sh hellofresh_gb_products_a  # copy one index
#
set -euo pipefail
...
copy_index() {
  ...
  # Keep only transferable settings; 0 replicas for single-node local cluster.
  ...
  local dst_exists
  dst_exists=$(curl -s -o /dev/null -w "%{http_code}" "${DST_URL}/${index}")
  ...
  while [[ "${hits}" -gt 0 ]]; do
    ...
    # Send bulk request (use raw curl, not dst_curl, to set correct Content-Type)
    local bulk_result
    bulk_result=$(curl -s \
